...
client.on('message', message => {
	...
	if(message.channel.name == "discord-commands") {
		...
		message.delete(300000);
			...
			if (message.mentions.users.first().roles.has(message.guild.roles.find(r => r.name.toLowerCase() === "notifications"))) {
			...
			else { message.channel.send(":no_entry: User has notifications disabled!").then(msg => {msg.delete(4000)}); }
			message.delete();
		}
		else {
			message.delete();
